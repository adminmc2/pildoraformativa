  ...
  return (
    <div className="w-full h-full flex items-center justify-center overflow-y-auto md:overflow-hidden">
      <div className="w-full max-w-[1800px] grid grid-cols-1 md:grid-cols-[2.5fr_1fr] gap-4 md:gap-6 items-center px-3 md:px-4">
        <div className="flex flex-col gap-1.5 min-w-0" style={{ height: "calc(100vh - 80px)" }}>
          {/* Badge */}
          <div className="flex items-center gap-3">
            ...
            <span
              ...
              style={{ fontSize: "clamp(18px, 1.3vw, 20px)", background: "var(--color-pf-pill-soft)", color: "#3F6B14" }}
            >
              Método
            </span>
          </div>
          ...
          {visibleCount > 0 && (
            <div
              className="rounded-[16px] bg-white shadow-[0_10px_30px_-12px_rgba(0,0,0,0.12)] overflow-hidden flex-1 min-h-0 overflow-y-auto"
              style={{
                animation:
                    ...
                    <React.Fragment key={i}>
                      {showTheme && (
                        <div
                          ...
                          style={{ fontSize: "clamp(20px, 1.6vw, 24px)", background: t.soft, color: t.color }}
                        >
                          {t.label}
                        </div>
                      )}
                      ...
                      >
                        <span
                          ...
                          style={{ fontSize: "clamp(20px, 1.6vw, 24px)", color: t.color }}
                        >
                          {row.marta}
                        </span>
                        <ArrowRight
                        ...
                        />
                        <span
                          className="leading-snug text-[var(--color-pf-ink)] opacity-70 italic"
                          style={{ fontSize: "clamp(20px, 1.6vw, 24px)" }}
                        >
                          {row.hint}
                        </span>
          ...
          <div className="flex items-center gap-3 mt-1">
            <button
              ...
              disabled={!canPrev}
              className="w-11 h-11 rounded-full bg-white/80 text-[var(--color-pf-ink)] font-bold disabled:opacity-30 hover:bg-white transition flex-shrink-0"
              style={{ fontSize: "clamp(20px, 1.6vw, 24px)" }}
              aria-label="Atrás"
            >
              ←
            ...
            <button
              ...
              disabled={!canNext}
              className="px-7 rounded-full bg-[var(--color-pf-ink)] text-white font-[family-name:var(--font-pf-display)] disabled:opacity-40 hover:opacity-90 transition min-h-[44px]"
              style={{ fontSize: "clamp(20px, 1.6vw, 24px)", animation: step === 0 ? "btnPulse 2s ease-in-out infinite" : undefined }}
            >
              {step === 0
                ? "EMPEZAR"
                : step === totalSteps
